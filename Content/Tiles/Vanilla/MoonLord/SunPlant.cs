...
internal class SunPlant : ModTile
{
    ...
    public class SunPlantTE : ModTileEntity
    {
        public List<(SunPlantSegment segmentType, float length)> Segments = [];

        public override bool IsTileValidForEntity(int x, int y) => Main.tile[x, y].TileType == ModContent.TileType<SunPlant>();
        ...
        public override void Update()
        {
            if (Segments.Count == 0)
            {
                int segments = Main.rand.Next(3, 11);
            ...
            static float Step(int minSteps, int maxSteps) => Main.rand.Next(minSteps, maxSteps) * 0.5f;
        }
